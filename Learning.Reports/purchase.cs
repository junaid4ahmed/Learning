  ...
  public class purchase
    ...
    public void select() {
      ...
      .Select(pur_sel => new {
        ...
        products = pur_sel.purchase_items.Select(ite_sel => new { id = ite_sel.purchase_item_id, ite_sel.product.name, ite_sel.quantity, ite_sel.unit_cast, ite_sel.inventoried, total = (ite_sel.quantity * ite_sel.unit_cast) }),
      });

      foreach(var pur in pur_col) {
        Console.WriteLine($"-{pur.id} {pur.status_id} {pur.sub_total}");
        foreach(var ite in pur.products) {
          Console.WriteLine($"--{ite.id } {ite.name} {ite.quantity} {ite.unit_cast} {ite.total}, { ite.inventoried }");
        }
      }
      Console.WriteLine();
    }
