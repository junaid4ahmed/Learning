...
namespace Learning.Reports {
  public class payment
    ...
    public void select(int account_id) {
      ...
      foreach (Model.Entities.payment item in payments) {
        Console.WriteLine($"--{item.id}, { item.account_id }, { item.payment_method.name }, { item.amount }, { item.date }  ");
      }
      Console.WriteLine();
    }
    ...
    public void select() {
      Console.WriteLine("payments");
      var payments = _context.Payments.Include("payment_method").Include("account");
      //total = temp.purchase_items.Count != 0 ? temp.purchase_items.Sum(pi => pi.quantity * pi.unit_cast) : 0.0m;

      decimal total = payments.Count() != 0 ? payments.Sum(p => p.amount) : 0.0m; 
      Console.WriteLine($"-total { total }");
      foreach(Model.Entities.payment item in payments) {
        Console.WriteLine($"--{item.id}, { item.account.name }, { item.payment_method.name }, { item.amount }, { item.date }  ");
      }
      Console.WriteLine();
    }
